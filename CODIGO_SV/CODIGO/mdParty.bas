...
'***************************************************
    ...
    ElseIf CInt(UserList(UserIndex).Stats.UserAtributos(eAtributos.Carisma)) * UserList(UserIndex).Stats.UserSkills(eSkill.Liderazgo) < 100 Then
        Call WriteConsoleMsg(UserIndex, "Tu carisma y liderazgo no son suficientes para liderar una party.", FontTypeNames.FONTTYPE_PARTY, eMessageType.Party)
        PuedeCrearParty = False
    ...
    If .PartyIndex = 0 Then
        If .flags.Muerto = 0 Then
            If .Stats.UserSkills(eSkill.Liderazgo) >= 5 Then
                ...
                End If
            Else
                Call WriteConsoleMsg(UserIndex, "No tienes suficientes puntos de liderazgo para liderar una party.", FontTypeNames.FONTTYPE_PARTY, eMessageType.Party)
            End If
        Else
            Call WriteConsoleMsg(UserIndex, "¡¡Estás muerto!!", FontTypeNames.FONTTYPE_PARTY)
        End If
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If (.flags.Privilegios And PlayerType.User) = 0 Then
            Call WriteConsoleMsg(UserIndex, "¡Los miembros del staff no pueden unirse a partys!", FontTypeNames.FONTTYPE_PARTY, eMessageType.Party)
            Exit Sub
        End If
        
        If .PartyIndex > 0 Then
            'si ya esta en una party
            Call WriteConsoleMsg(UserIndex, "Ya perteneces a una party, escribe /SALIRPARTY para abandonarla", FontTypeNames.FONTTYPE_PARTY, eMessageType.Party)
            .PartySolicitud = 0
            Exit Sub
        End If
        ...
        If TargetUserIndex > 0 Then
            ...
            If PartyIndex > 0 Then
                ...
                If Parties(PartyIndex).EsPartyLeader(TargetUserIndex) Then
                    ...
                    Call WriteConsoleMsg(UserIndex, "El lider decidirá si te acepta en la party.", FontTypeNames.FONTTYPE_PARTY, eMessageType.Party)
                    Call WriteConsoleMsg(TargetUserIndex, .Name & " solicita ingresar a tu party.", FontTypeNames.FONTTYPE_PARTY, eMessageType.Party)
                
                ' No es lider
                Else
                    Call WriteConsoleMsg(UserIndex, UserList(TargetUserIndex).Name & " no es lider de la party.", FontTypeNames.FONTTYPE_PARTY, eMessageType.Party)
                End If
            
            ' No tiene party
            Else
                Call WriteConsoleMsg(UserIndex, UserList(TargetUserIndex).Name & " no pertenece a ninguna party.", FontTypeNames.FONTTYPE_PARTY, eMessageType.Party)
                .PartySolicitud = 0
                Exit Sub
            End If
        ...
        ' Target inválido
        Else
            Call WriteConsoleMsg(UserIndex, "Para ingresar a una party debes hacer click sobre el fundador y luego escribir /PARTY", FontTypeNames.FONTTYPE_PARTY, eMessageType.Party)
            .PartySolicitud = 0
        End If
...
'***************************************************
    ...
    With UserList(NewMember)
            ...
            If Not .flags.Muerto = 1 Then
                If .PartyIndex = 0 Then
                    If Parties(PI).PuedeEntrar(NewMember, razon) Then
                        ...
                        Else
                        'no debe entrar
                        Call WriteConsoleMsg(leader, razon, FontTypeNames.FONTTYPE_PARTY, eMessageType.Party)
                    End If
                Else
                    If .PartyIndex = PI Then
                        Call WriteConsoleMsg(leader, LCase(.Name) & _
                            " ya es miembro de la party.", FontTypeNames.FONTTYPE_PARTY, eMessageType.Party)
                    Else
        ...
        Else
            ...
            Else
                Call WriteConsoleMsg(leader, LCase(.Name) & _
                    " no ha solicitado ingresar a tu party.", FontTypeNames.FONTTYPE_PARTY, eMessageType.Party)
            End If
