...
Public Sub LoadGuildsDB()
    ...
    For i = 1 To CANTIDADDECLANES
        ...
        secStr = GetVar(GUILDINFOFILE, "GUILD" & i, "SECGUILDNAME")
        Alin = String2Alineacion(GetVar(GUILDINFOFILE, "GUILD" & i, "Alineacion"))
        Call guilds(i).Inicializar(TempStr, i, Alin, TempStr)
    Next i
...
Public Sub ChangeAlineation_Enlist(ByVal UserIndex As Integer, _
                                   ByVal Alineation As ALINEACION_GUILD)
    
    With UserList(UserIndex)

        If .GuildIndex > 0 Then
            If m_EsGuildLeader(.Name, .GuildIndex) Then
                Call UpdateGuildMembers(.GuildIndex, Alineation)
            End If
        End If
...
Public Function m_ValidarPermanencia(ByVal UserIndex As Integer, _
    ...
    If Not m_EstadoPermiteEntrar(UserIndex, GuildIndex) Then
        ...
        If m_EsGuildLeader(UserList(UserIndex).Name, GuildIndex) Then '
            Call LogClanes(UserList(UserIndex).Name & ", líder de " & guilds(GuildIndex).SecGuildName & " hizo bajar la alienación de su clan.")
            
            CambioAlineacion = True
            
            Call UpdateGuildMembers(GuildIndex, ALINEACION_NEUTRO)

        Else
            Call LogClanes(UserList(UserIndex).Name & " de " & guilds(GuildIndex).SecGuildName & " es expulsado en validar permanencia.")
            ...
            'Call LogClanes(UserList(UserIndex).Name & " de " & guilds(GuildIndex).SecGuildName & _
...
Private Sub UpdateGuildMembers(ByVal GuildIndex As Integer, _
                               ByVal Alineation As ALINEACION_GUILD)
    ...
    Call guilds(GuildIndex).CambiarAlineacion(Alineation)
    ...
    For MemberIndex = 0 To TotalMembers
        ...
        If Not m_EsGuildLeader(MemberName, GuildIndex) Then
    ...
    If UserIndex > 0 Then
        ...
        If GI > 0 Then
            If m_PuedeSalirDeClan(Expulsado, GI, Expulsador) Then
                ...
                Call LogClanes(Expulsado & " ha sido expulsado de " & guilds(GI).SecGuildName & " Expulsador = " & Expulsador)
...
Public Function CrearNuevoClan(ByVal FundadorIndex As Integer, _
                               ...
                               ByRef codex() As String, _
                               ByRef refError As String, _
                               ByRef SecGuildName As String, _
                               ByVal GuildAlineation As ALINEACION_GUILD) As Boolean
    '***************************************************
    'Author: Unknown
    'Last Modification: -
    '
    ...
    If Len(GuildName) > 15 Then
        refError = "El clan debe tener 15 caracteres o menos."
    ...
    If Not Guild_CheckFundationAlineation(FundadorIndex, GuildAlineation, DummyString) Then
        refError = DummyString
    ...
    If CANTIDADDECLANES < UBound(guilds) Then
        ...
        With guilds(CANTIDADDECLANES)
            Call .Inicializar(GuildName, CANTIDADDECLANES, GuildAlineation, SecGuildName)
            ...
            Call .SetDesc(Desc)
            Call .SetGuildNews("Clan creado con alineación: " & Alineacion2String(GuildAlineation))
        ...
        UserList(FundadorIndex).Stats.Gld = UserList(FundadorIndex).Stats.Gld - 25000000
        WriteUpdateGold (FundadorIndex)
    Else
...
Public Sub SendGuildNews(ByVal UserIndex As Integer)
    ...
    With guilds(GuildIndex)
        ...
        If .EleccionesAbiertas Then
            Call WriteConsoleMsg(UserIndex, "Hoy es la votación para elegir un nuevo líder para el clan.", FontTypeNames.FONTTYPE_GUILD, eMessageType.guild)
            Call WriteConsoleMsg(UserIndex, "La elección durará 24 horas, se puede votar a cualquier miembro del clan.", FontTypeNames.FONTTYPE_GUILD, eMessageType.guild)
            Call WriteConsoleMsg(UserIndex, "Para votar escribe /VOTO NICKNAME.", FontTypeNames.FONTTYPE_GUILD, eMessageType.guild)
            Call WriteConsoleMsg(UserIndex, "Sólo se computará un voto por miembro. Tu voto no puede ser cambiado.", FontTypeNames.FONTTYPE_GUILD, eMessageType.guild)
        End If
...
Public Function Guild_CheckFundationAlineation(ByVal UserIndex As Integer, _
                                               ByVal Alineation As ALINEACION_GUILD, _
    ...
    Guild_CheckFundationAlineation = True
...
Public Function PuedeFundarUnClan(ByVal UserIndex As Integer, _
    ...
    If UserList(UserIndex).Stats.Elv < 35 Then
        refError = "Para fundar clan debes ser nivel 35."

        Exit Function

    End If
    
    If (UserList(UserIndex).Stats.Gld < 25000000) Or TieneObjetos(886, 1, UserIndex) = False Or TieneObjetos(887, 1, UserIndex) = False Or TieneObjetos(888, 1, UserIndex) = False Or (UserList(UserIndex).Stats.UserSkills(eSkill.Liderazgo) < 100) Then
...
Private Function m_EstadoPermiteEntrarChar(ByRef Personaje As String, _
    ...
    If FileExist(CharPath & Personaje & ".chr") Then
        ...
        Select Case guilds(GuildIndex).Alineacion
            ...
            Case ALINEACION_GUILD.ALINEACION_ARMADA
                m_EstadoPermiteEntrarChar = CByte(GetVar(CharPath & Personaje & ".chr", "FACTION", "STATUS")) = eTipoFaction.r_Armada
            
            Case ALINEACION_GUILD.ALINEACION_NEUTRO
                m_EstadoPermiteEntrarChar = CByte(GetVar(CharPath & Personaje & ".chr", "FACTION", "STATUS")) = eTipoFaction.r_None
                
            Case ALINEACION_GUILD.ALINEACION_LEGION
                m_EstadoPermiteEntrarChar = CByte(GetVar(CharPath & Personaje & ".chr", "FACTION", "STATUS")) = eTipoFaction.r_Caos
...
Private Function m_EstadoPermiteEntrar(ByVal UserIndex As Integer, _
    ...
    Select Case guilds(GuildIndex).Alineacion
        ...
        Case ALINEACION_GUILD.ALINEACION_ARMADA
            m_EstadoPermiteEntrar = Not criminal(UserIndex) And UserList(UserIndex).Faction.Status = r_Armada
        
        Case ALINEACION_GUILD.ALINEACION_LEGION
            m_EstadoPermiteEntrar = criminal(UserIndex) And UserList(UserIndex).Faction.Status = r_Caos
        
        Case ALINEACION_GUILD.ALINEACION_NEUTRO
            m_EstadoPermiteEntrar = (Not UserList(UserIndex).Faction.Status = r_Armada And Not UserList(UserIndex).Faction.Status = r_Caos)
...
Private Function GuildNameValido(ByVal cad As String) As Boolean
    ...
    For i = 1 To Len(cad)
        ...
        If (car < 97 Or car > 122) And (car <> 32) And (car <> 36) And (car <> 40) And (car <> 44) And (car <> 46) And (car <> 59) Then
...
Public Sub v_RutinaElecciones()
    ...
    Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor> Revisando elecciones", FontTypeNames.FONTTYPE_SERVER, eMessageType.Admin))

    For i = 1 To CANTIDADDECLANES

        If Not guilds(i) Is Nothing Then
            If guilds(i).RevisarElecciones Then
                Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor> " & guilds(i).GetLeader & " es el nuevo líder de " & guilds(i).SecGuildName & ".", FontTypeNames.FONTTYPE_SERVER, eMessageType.Admin))
            End If
        End If
...
proximo:
    ...
    Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor> Elecciones revisadas.", FontTypeNames.FONTTYPE_SERVER, eMessageType.Admin))
...
Public Function m_ListaDeMiembrosOnline_Fast(ByVal GuildIndex As Integer) As String
    ...
    If GuildIndex > 0 And GuildIndex <= CANTIDADDECLANES Then
        ...
        While i > 0
            ...
            m_ListaDeMiembrosOnline_Fast = m_ListaDeMiembrosOnline_Fast & UserList(i).Name & ","
    ...
    If Len(m_ListaDeMiembrosOnline_Fast) > 0 Then
        m_ListaDeMiembrosOnline_Fast = Left$(m_ListaDeMiembrosOnline_Fast, Len(m_ListaDeMiembrosOnline_Fast) - 1)
    End If
...
Public Function m_ListaDeMiembrosOnline(ByVal UserIndex As Integer, _
                                        ByVal GuildIndex As Integer, _
                                        Optional ByVal ToCVC As Boolean = False) As String
    '***************************************************
    'Author: Unknown
...
Public Function m_ListaDeMiembrosOnline_CVC(ByVal UserIndex As Integer, _
                                            ByVal GuildIndex As Integer, _
                                            Optional ByVal MaxUser As Byte = 0) As String
    '***************************************************
    'Author: Unknown
    ...
    If GuildIndex > 0 And GuildIndex <= CANTIDADDECLANES Then
        ...
        While (i > 0)
            ...
            If CVC_CanUserFight(i, False) Then
                If (MaxUser = 0) Then
                    m_ListaDeMiembrosOnline_CVC = m_ListaDeMiembrosOnline_CVC & UserList(i).Name & ","
                Else
                    Amount = Amount + 1
                    
                    If Amount < MaxUser Then
                        m_ListaDeMiembrosOnline_CVC = m_ListaDeMiembrosOnline_CVC & UserList(i).Name & ","
                    End If
                End If
            End If
    ...
    If Len(m_ListaDeMiembrosOnline_CVC) > 0 Then
        m_ListaDeMiembrosOnline_CVC = Left$(m_ListaDeMiembrosOnline_CVC, Len(m_ListaDeMiembrosOnline_CVC) - 1)
    End If
...
Public Function PrepareGuildsList() As String()
    ...
    Else
        ...
        For i = 1 To CANTIDADDECLANES
            tStr(i - 1) = guilds(i).SecGuildName
        Next i
...
Public Function GMEscuchaClan(ByVal UserIndex As Integer, _
    ...
    If LenB(GuildName) = 0 And UserList(UserIndex).EscucheClan <> 0 Then
        ...
        Call WriteConsoleMsg(UserIndex, "Dejas de escuchar a : " & guilds(UserList(UserIndex).EscucheClan).SecGuildName, FontTypeNames.FONTTYPE_GUILD)
...
Public Function r_RechazarPropuestaDeAlianza(ByVal UserIndex As Integer, _
    ...
    Call guilds(GIG).SetGuildNews(guilds(GI).SecGuildName & " ha rechazado nuestra propuesta de alianza. " & guilds(GIG).GetGuildNews())
...
Public Function r_RechazarPropuestaDePaz(ByVal UserIndex As Integer, _
    ...
    Call guilds(GI).AnularPropuestas(GIG)
    'avisamos al otro clan
    Call guilds(GIG).SetGuildNews(guilds(GI).SecGuildName & " ha rechazado nuestra propuesta de paz. " & guilds(GIG).GetGuildNews())
    ...
    If GI > 0 And GI <= CANTIDADDECLANES Then
        ...
        With guilds(GI)
            ...
            For i = 0 To proposalCount - 1
                proposals(i) = guilds(.Iterador_ProximaPropuesta(Tipo)).SecGuildName
            Next i
...
Public Function a_RechazarAspirante(ByVal UserIndex As Integer, _
    ...
    refError = "Fue rechazada tu solicitud de ingreso a " & guilds(GI).SecGuildName
...
Public Sub SendDetallesPersonaje(ByVal UserIndex As Integer, ByVal Personaje As String)
    ...
    With UserFile
        ...
        If GuildActual > 0 And GuildActual <= CANTIDADDECLANES Then
            GuildName = "<" & guilds(GuildActual).SecGuildName & ">"
        Else
            GuildName = "Ninguno"
        End If
        ...
        Call Protocol.WriteCharacterInfo(UserIndex, Personaje, .GetValue("INIT", "Raza"), .GetValue("INIT", "Clase"), .GetValue("INIT", "Genero"), .GetValue("STATS", "ELV"), .GetValue("STATS", "GLD"), .GetValue("STATS", "Banco"), .GetValue("REP", "Promedio"), .GetValue("GUILD", "Pedidos"), GuildName, Miembro, (.GetValue("FACTION", "STATUS") = 1), (.GetValue("FACTION", "STATUS") = 2), .GetValue("FACTION", "FRAGSCIU"), .GetValue("FACTION", "FRAGSCRI"))
    End With
...
Public Function SecGuildName(ByVal GuildIndex As Integer) As String
    '***************************************************
    'Author: Unknown
    'Last Modification: -
    '
    ...
    SecGuildName = guilds(GuildIndex).SecGuildName
