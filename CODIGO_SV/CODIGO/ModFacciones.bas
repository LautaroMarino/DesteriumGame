...
With UserList(UserIndex)
    ...
    If .Faction.FragsCri < 30 Then
        Call WriteChatOverHead(UserIndex, "Para unirte a nuestras fuerzas debes matar al menos 30 criminales, sólo has matado " & .Faction.FragsCri & ".", str(Npclist(.flags.TargetNPC).Char.CharIndex), vbWhite)
        Exit Sub
    End If
    ...
    If .Faccion.RecibioArmaduraReal = 0 Then
        ...
        .Faccion.MatadosIngreso = .Faction.FragsCiu
...
With UserList(UserIndex)
    ...
    If .Faction.FragsCiu < 70 Then
        Call WriteChatOverHead(UserIndex, "Para unirte a nuestras fuerzas debes matar al menos 70 ciudadanos, sólo has matado " & .Faction.FragsCiu & ".", str(Npclist(.flags.TargetNPC).Char.CharIndex), vbWhite)
        Exit Sub
    End If
